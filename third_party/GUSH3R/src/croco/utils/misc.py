    ...
    if args.resume is not None:
        ...
        if "optimizer" in checkpoint:
            try:
                ...
                # If parameter groups changed, resume with a fresh optimizer.
                printer.warning(
                    "Skipping optimizer state (parameter groups mismatch): %s", exc
                )
        if "scaler" in checkpoint:
            try:
                loss_scaler.load_state_dict(checkpoint["scaler"])
            except (ValueError, RuntimeError) as exc:
                printer.warning("Skipping scaler state: %s", exc)
